...
int server_create( webserver* server, char* confile )
{
	...
	server->max_onlines = xml_readnum(conf, "maxOnline");
	...
	NEW( server->thread_list, sizeof(pthread_t)*server->max_threads ); 
...
static void* server_listen( void* data )
{
	...
	while( server->state == S_RUNNING )
	{
		...
		int len = sizeof(sa);
		uint ip;
		getpeername(sock, (struct sockaddr *)&sa, (int*)&len);
...
int server_start( webserver* server )
{
	...
	pthread_attr_setstacksize(&attr, 256*1024); //64KB enough??
...
int server_stop( webserver* server )
{
	...
	for( i=0; i<server->max_threads; i++ )
		pthread_join( server->thread_list[i], (void**)&ret );
